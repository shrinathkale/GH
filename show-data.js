document.getElementById('guestsAvailable').addEventListener('click', async () => {
    try {
      const res = await fetch('http://localhost:3000/api/get-guests'); // adjust port if needed
...
document.getElementById('guestsAll').addEventListener('click', async () => {
    try {
      const res = await fetch('http://localhost:3000/api/get-guestsAll'); // adjust port if needed
      const data = await res.json();
